    ...
    var playButton = document.getElementById('btn-play');
        playButton.addEventListener('click', function(e) {
            ...
            if (hero.name.length === 0) {
                ...
                alert('Name Should not Be Longer Than 20 Characters !!');
            }
...
function displayBestiary() {
    ...
    for (var i = 0; i < options.enemiesEncountered.length; i++) {
        ...
        list += '<p> Health : ' + monster.health + '</p>';
        list += '<p> Type : ' + monster.moveType + '</p>';
...
function listFallenStats(hero,view) {
    ...
    var stats = '<h5>' + hero.name + '</h5>';
        ...
        stats += '<p>Dungeon Floor : <span>' + hero.gameLevel + '</span></p>';
        ...
        stats += '<p>Enemies Slained : <span>' + hero.enemiesSlain + '</span></p>';
        ...
        stats += '<p>Times Webbed : <span>' + hero.timesWebbed + '</span></p>';
...
function openStore() {
    ...
    var store = '<h5>- Avatar Store -</h5><br />';
        store += '<p>Put your hard earned gold to good use with these items !!<br />  Or don&apos;t, It&apos;s your gold</p><br />';
...
function addHero() {
    ...
    var level = document.getElementById('level');
        level.innerHTML = 'Dungeon Floor ' + hero.gameLevel;
    ...
    if (options.tutorial === false && options.endgame === false) {
        // Start spawning enemies once the player has started moving or after 8 seconds because these monster aint got all day.
        var moves = hero.squaresMoved;
        var timer = 0;
        var interval = setInterval(function() {
            if (hero.squaresMoved > moves || timer >= 8) {
                clearInterval(interval);
                letTheGamesBegin();
            }
            timer++;
...
var rescueText =    ['You&apos;re here to rescue us? Inconceivable !!',
                    'You are the greatest knight that ever lived !!',
                    'Thank you !!',
...
function startTutorial() {
    ...
    var message = 'Welcome to Dungeon Maths !!<br />Tap here to get started';
    textBubble(message);
    hero.evasion = 100;
    hero.canCapture = false;
    ...
    function teachTraps() {
        ...
        var takeDamage = setInterval(function() {
            if (hero.health < 100) {
                ...
                var message = 'Ouch!! Break open one of these containers by constantly trying to step on them and find something to heal yourself with';
    ...
    function teachMath() {
        var message = 'What would a dungeon maths game be without maths, right ?';
    ...
    function teachCombat() {
        ...
        var interval = setInterval(function() {
            if (tutorialData.proceed) {
                ...
                clearInterval(interval);
                var message = 'Stand next to an enemy and tap on it to attack but Watch out, because different enemies have different abbilities !!';
        ...
        var slayMonster = setInterval(function() {
            if (hero.enemiesSlain > 0) {
                hero.canCapture = true;
                var message = "To proceed deeper into the dungeon capture any remaining tiles which are multiples of 10";
                textBubble(message,0);
    ...
    function finishUp() {
        var interval = setInterval(function() {
            if (hero.answers >= hero.answersNeeded) {
                var message = ['Your training is complete, but your journey is just beginning...',
                    'You are the only remaining Knight. The others have all been captured by the foul beasts within this dungeon',
        ...
        document.getElementById('enable-tutorial').checked = false;
    }
}
/////////////// DEATH ///////////////
